      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-[var(--admin-text-muted)] mt-1">
            Review and respond to incoming quote inquiries.
          </p>
        </div>
          ...
          <table className="w-full text-left min-w-[1000px]">
            <thead>
              <tr className="border-b border-[var(--admin-border)] bg-black/5 dark:bg-white/5">
                <th className="px-6 py-4 text-xs font-mono uppercase tracking-widest text-[var(--admin-text-muted)]">
                  Customer
                </th>
                <th className="px-6 py-4 text-xs font-mono uppercase tracking-widest text-[var(--admin-text-muted)]">
                  Email
            ...
            <tbody className="divide-y divide-[var(--admin-border)]">
              {loading ? (
                ...
                <tr>
                  <td
                    colSpan={8}
                    className="px-6 py-12 text-center text-[var(--admin-text-muted)] font-medium"
                  >
                  ...
                  >
                      ...
                      <div className="flex flex-wrap gap-1">
                        {quote.services?.slice(0, 2).map((s, idx) => (
                          <Badge
                            key={idx}
                            variant="outline"
                            className="text-[10px] bg-black/5 dark:bg-white/5 border-[var(--admin-border)] text-[var(--admin-text)]"
                          >
                            {s}
                          </Badge>
                        ))}
                        {quote.services?.length > 2 && (
                          <Badge
                            variant="outline"
                            className="text-[10px] bg-black/5 dark:bg-white/5 border-[var(--admin-border)] text-[var(--admin-text-muted)]"
                          >
                    ...
                    <td className="px-6 py-5">
                      {quote.status === "replied" ? (
                        ...
                        <Badge
                          variant="outline"
                          className="text-[var(--admin-text-muted)] border-[var(--admin-border)] font-medium"
                        >
                          New
                        </Badge>
                      )}
                      ...
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button
                            variant="ghost"
                            size="icon"
                            className="text-[var(--admin-text-muted)] hover:text-[var(--admin-text)] hover:bg-black/5 dark:hover:bg-white/5 rounded-lg"
                          >
                        ...
                        <DropdownMenuContent
                          align="end"
                          className="bg-[var(--admin-sidebar)] border-[var(--admin-border)] text-[var(--admin-text)]"
                        >
                          <DropdownMenuItem
                            onClick={() => updateStatus(quote.id, "replied")}
                            className="hover:bg-emerald-500/10 focus:bg-emerald-500/10 text-emerald-600"
                          >
